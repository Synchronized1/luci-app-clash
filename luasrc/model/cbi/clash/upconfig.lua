...
local e,a={}
for t,o in ipairs(fs.glob("/usr/share/clash/config/upload/*"))do
a=fs.stat(o)
if a then
e[t]={}
e[t].name=fs.basename(o)
e[t].mtime=os.date("%Y-%m-%d %H:%M:%S",a.mtime)
if string.sub(luci.sys.exec("uci get clash.config.config_path_up"), 23, -2) == e[t].name then
...
luci.sys.exec(string.format('uci set clash.config.config_up_name="%s"',e[t].name))
