...
function index()
	...
	entry({"admin", "services", "clash", "delog"}, call("del_conf")).leaf=true
	entry({"admin", "services", "clash", "apply"}, call("sub_save")).leaf=true
...
function geoip_update()
	...
	luci.sys.exec("kill $(pgrep /usr/share/clash/geoip.sh) ; (rm /var/run/geoip_update_error ;  touch /var/run/geoip_update ; sh /usr/share/clash/geoip.sh >/tmp/geoip_update.txt 2>&1  || touch /var/run/geoip_update_error ;rm /var/run/geoip_update) &")
end


function do_update()
	fs.writefile("/var/run/clashlog","0")
	luci.sys.exec("kill $(pgrep sh /usr/share/clash/core_download.sh) ; (rm /var/run/core_update_error ;  touch /var/run/core_update ; sh /usr/share/clash/core_download.sh >/tmp/clash_update.txt 2>&1  || touch /var/run/core_update_error ;rm /var/run/core_update) &")
end
